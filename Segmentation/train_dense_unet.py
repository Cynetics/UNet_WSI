...
if __name__ == '__main__':
    ...
    net = DenseUnet(num_filters=config.n_filters,
                                  ...
                                  latent_hidden_n=config.latent_hidden_n,
    ...
    if config.model_checkpoint is not None:
        ...
        print('Model loaded: {}'.format(config.model_checkpoint))
    net.to(device)
    #cudnn.benchmark = True # faster convolutions, but more memory
    train_net(net, training_loader, val_loader, config, save_cp=True, model_name="dense_10x_revised_bolero_test_oversampling")
